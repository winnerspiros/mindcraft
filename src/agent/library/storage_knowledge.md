...
- CHEST — the basic box. 27 slots. Craft: 8 planks in a ring (any wood, middle empty).
  ...
  !viewChest. Place two side-by-side for one big 54-slot double chest.
- TRAPPED CHEST — a chest that also sends a redstone signal when opened. Craft: 1 chest +
  1 tripwire_hook (tripwire_hook = 1 iron_ingot + 1 stick + 1 plank). Use: storage PLUS a
  silent alarm — wire it to a trap/light that triggers the moment someone snoops.
- BARREL — storage just like a chest (27 slots) but opens from the TOP, so it still works
  with a solid block directly above it. Craft: 6 planks (top+bottom rows) + 2 wood_slab
  (middle-left + middle-right). Use: store stuff where a chest lid would be blocked.
- SHULKER BOX — portable 27-slot storage that KEEPS its items when you break/pick it up.
  Craft: 1 chest + 2 shulker_shell (shapeless). Get: kill shulkers in End City ships (the
  End), or combine shells from those kills. Use: your backpack — fill it, carry it, place
  it, break it to take the whole load with you. Dyeable.
- ENDER CHEST — 27 slots that are SHARED across every ender chest everywhere, and are
  yours alone (per-player; never drops on death). Craft: 8 obsidian in a ring + 1
  eye_of_ender in the middle (eye_of_ender = blaze_powder + ender_pearl). Get: obsidian =
  mine water+lava with a diamond pickaxe; blaze powder from blazes (Nether); ender pearls
  from endermen. Use: your personal vault that follows you to any ender chest.
